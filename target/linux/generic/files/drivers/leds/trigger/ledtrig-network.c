		...
		if (id >= 0) {
			...
			if (tmp) {
				...
				if (m->dev_slot_limit == id + 1) {
					/* shrink dev_slot_limit through NULL holes */
					while (m->dev_slot_limit > 0 && !m->devs[m->dev_slot_limit - 1])
						m->dev_slot_limit--;
				}
				mutex_unlock(&m->lock);
				dev_put(tmp);
...
static struct net_mgr *net_mgr_get(enum net_trig_type type)
{
	...
	if (existing) {
		...
		/* clean up our temporary manager properly (dev_put any held devs etc.) */
		net_mgr_put(m);
...
static ssize_t net_flag_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	/* obtain per-LED trigger data using helper (no dev_get_drvdata + trigger_data) */
...
static ssize_t net_dev_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if (entry->mgr->type >= NET_TRIG_TYPE_MAX)
		return -EINVAL;
...
{
	struct net_led *entry = led_trigger_get_drvdata(led_cdev->dev);
	struct net_mgr *m;
	...
	if (!m) {
		...
		dev_set_drvdata(led_cdev->dev, NULL);
		kfree(entry);
		return;
	}
	mutex_lock(&m->lock);
...
static int net_activate(struct led_classdev *led_cdev)
{
	...
	dev_set_drvdata(led_cdev->dev, entry);
...
static struct led_trigger network_trigger = {
	...
	/* Use .groups + led_trigger_get_drvdata/dev_set_drvdata; avoid device_create_file/remove_file. */
